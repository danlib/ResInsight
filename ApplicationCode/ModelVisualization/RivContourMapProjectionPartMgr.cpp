...
cvf::ref<cvf::Part> RivContourMapProjectionPartMgr::createProjectionMapPart(const caf::DisplayCoordTransform* displayCoordTransform) const
{
    ...
    {
        cvf::Vec3d vertex(vertices[i].x(), vertices[i].y(), vertices[i].z());
        cvf::Vec3f displayVertexPos (displayCoordTransform->transformToDisplayCoord(vertex));
    ...
    for (int64_t i = (int64_t) contourPolygons.size() - 1; i >= 0; --i)
    {
        ...
        for (size_t j = 0; j < contourPolygons[i].size(); ++j)
        {
            ...
            for (size_t l = 0; l < nLabels; ++l)
            {
                ...
                labelVertex.z() += 3.0f;
                label->addText(cvf::String(contourPolygons[i][j].value), labelVertex);
                bool overlaps = false;
